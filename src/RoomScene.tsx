...
import { OrbitControls, Environment, ContactShadows, Text, useGLTF, Float } from "@react-three/drei";
  ...
  return (
    <group>
      ...
      <mesh position={[0, -0.1, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#16233a" roughness={0.9} />
      </mesh>

      {/* Left Wall (Poster side) */}
      <mesh position={[0, 1.8, -3.6]} receiveShadow>
        <boxGeometry args={[7, 4.0, 0.2]} />
        <meshStandardMaterial color="#3a4a64" roughness={0.85} />
      </mesh>
      ...
      <mesh position={[-3.5, 3.8, 0]}>
        ...
        <meshStandardMaterial color="#28334a" />
      </mesh>
...
function SceneLights() {
  ...
  return (
    <>
      <ambientLight intensity={0.28} color="#1a2240" />
      {/* Sky: mid-blue  |  Ground: near-black navy  |  Gives everything a cool ambient fill */}
      <hemisphereLight args={["#3258c0", "#060c1e", 0.70]} />
      {/* Always-on fill — blue from high angle, gives premium depth even with all relays off */}
      <pointLight position={[4, 12, 4]} intensity={0.55} color="#223898" distance={28} decay={1.0} />
      {/* Always-on violet accent — subtle artistic rim from left */}
      <pointLight position={[-6, 7, 5]} intensity={0.30} color="#3a1a78" distance={22} decay={1.2} />
...
export function RoomScene({ modelUrl }: RoomSceneProps) {
  return (
    <Canvas
      shadows
      dpr={[1, 2]}
      camera={{ position: [11, 9, 11], fov: 46, near: 0.1, far: 120 }}
      style={{
        background: [
          "radial-gradient(ellipse at 46% 36%, #1c3868 0%, #0d1c3e 32%, #070e20 66%, #020508 100%)",
        ].join(", "),
      }}
    >
      <color attach="background" args={["#090f22"]} />
      <fog attach="fog" args={["#090f22", 18, 36]} />
      <SceneLights />

      <Suspense fallback={<CodedRoom />}>
        {modelUrl ? <RoomModel url={modelUrl} /> : <CodedRoom />}
      </Suspense>

      <TVScreen />

      <ContactShadows position={[0, -0.11, 0]} opacity={0.75} scale={14} blur={3} far={5} color="#00001a" />
